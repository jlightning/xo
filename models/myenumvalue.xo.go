...
func MyEnumValues(db XODB, schema string, tableName string, enum string) (*MyEnumValue, error) {
	var err error
	...
	// run query
	XOLog(sqlstr, schema, tableName, enum)
	var mev MyEnumValue
	err = db.QueryRow(sqlstr, schema, tableName, enum).Scan(&mev.EnumValues)
	if err != nil {
		return nil, err
	}
